...
* Standalone embed at /video-embed/ for places the plugin can't reach — locked templates, edge workers, other sites — iframe it or fetch and inline it.
...
= My homepage template won't render post content or shortcodes =

Use the standalone embed: `/video-embed/?layout=slider&count=8` serves just the video slider with no theme chrome. Iframe it from anywhere — `<iframe src="https://yoursite.org/video-embed/?layout=slider&count=8" style="width:100%;height:240px;border:0" loading="lazy" title="Recent videos"></iframe>` — or have an edge worker/CDN fetch and inline it. Tile links navigate the top window, and the page posts its height to the parent (`{ type: "svc-embed-height", height: N }`) if you want auto-resizing.
