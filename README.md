...
- **SEO** — VideoObject JSON-LD on single pages (with `contentUrl` direct .mp4 links on Vimeo plans that expose them), ItemList JSON-LD on the archive, a Google video sitemap at `/video-sitemap.xml` (advertised in robots.txt), and Open Graph / Twitter player tags so shared links unfurl as playable cards. Thumbnails are sideloaded into the media library.
